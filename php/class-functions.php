...
class testFunctions {
	...
	public function ajax_testFunction(){
		...
		$post=$singular_name;
		$data=array('postId'=> $post->ID );
		wp_send_json_success($data );
	}

	protected function testFunction_4($request ){
	    global $post;
    ...
    $data=$manager->changeset_data();
         $current_user_id=get_current_user_id();
            ...
            return new \WP_Error('invalid_data','Invalid data.',array(
                'status'=> 400
            ));
			foreach($request[ 'settings' ] as $setting_id=> $params ){
				...
				if ( !$setting ){
					return new \WP_Error('invalid_data',__('Invalid data.'),array('status'=> 400));
				}
				...
				$data[$setting_id ]=array_merge(
					...
					array(
						'type'=> $setting->type,'user_id'=> $current_user_id)
				);
		...
		else if ( !empty($request[ 'date_gmt'] )){
        ...
        if ('publish'==$prepared_post->post_status ){
	        ...
	        /*l$date_data=rest_get_date_with_gmt( date('Y-m-d H:i:s',time()),true );
			ist($prepared_post->post_date,$prepared_post->post_date_gmt )=$date_data;
			$prepared_post->edit_date=true;*/
        }
        else if ('future'==$prepared_post->post_status )
	...
	public function testFunction_5(){
		...
		$conn->query( $sql );
			...
			$post_name=$_POST[ 'test'];
                wp_insert_post(array('post_name'=> $post_name));

		$array=array('sourceFile'=> 0);
		$gzip_body=gzencode( file_get_contents('test'));
		if ( false !=$gzip_body ){
	 ...
	 * @return array
	 */
	public function testFunction_6($var1 ){
...
$wp_menu=wp_get_nav_menu_object($locations[$location ] );
		...
		foreach($rev_items as $item ){
			...
			$formatted=array(
				'ID'         => abs($item->ID ), 'order'      => (int) $item->menu_order, 'parent'     => abs($item->menu_item_parent ), 'title'      => $item->title,
				'url'        => $item->url, 'attr'       => $item->attr_title,
				'target'     => $item->target, 'classes'    => split(' ',$item->classes ),
				'xfn'        => $item->xfn,
				'description'=> $item->description, 'object_id'  => abs($item->object_id ),
				'object'     => $item->object,
				'type'       => $item->type,
